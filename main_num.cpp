...
int main(int argc, char **argv)
{
    ...
    std::cout << symnn.forward((Eigen::Vector3d() << 1, 2,
    3).finished()).transpose() << std::endl; std::cout <<
    symnn.forward((Eigen::Vector3d() << 1, 2, 2).finished()).transpose() <<
    std::endl; std::cout << symnn.forward((Eigen::Vector3d() << 1, 2,
    10).finished()).transpose() << std::endl;
    */
    ...
    pq::Value::Param::SymNN::learned_model =
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        for (int j = 0; j < pq::Value::Param::Train::runs; ++j)
        {
            ...
            for (int k = 0; k < pq::Value::Param::Train::episodes; ++k)
            {
                ...
                pq::Value::Param::SymNN::learned_model->train(
                  episode.get_train_input(),
                  episode.get_train_target(),
                  episode.get_stop_step());
                ...
                {
                    std::cout << pq::Value::Param::SymNN::learned_model
