...
namespace quadrotor
{
    Eigen::Vector<double, 6> dynamic_model_predict(const Eigen::Vector<double, 13> &state, const Eigen::Vector4d &controls, const Eigen::Vector<double, 23> &model_params)
    {
        ...
        Eigen::Matrix3d R = Eigen::Quaterniond(state[3], state[4], state[5], state[6]).toRotationMatrix();

        Eigen::Vector3d thrust = R.transpose() * model_params[0] * Eigen::Vector3d(0, 0, -model_params[22]) +
                                 (Eigen::Matrix<double, 3, 4>() << 0, 0, 0, 0,
                                  0, 0, 0, 0,
                                  model_params[20], model_params[20], model_params[20], model_params[20])
                                         .finished() *
                                     controls;
        double lKf = model_params[19] * model_params[20];
        Eigen::Vector3d torque = (Eigen::Vector3d() << lKf * (controls[1] - controls[3]),
                                  lKf * (controls[2] - controls[0]),
                                  model_params[21] * (controls[0] - controls[1] + controls[2] - controls[3]))
                                     .finished();
