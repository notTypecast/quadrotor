...
{
    Eigen::Matrix4d L;
    L << q.w(), -q.x(), -q.y(), -q.z(), q.x(), q.w(), -q.z(), q.y(), q.y(),
...
class Quadrotor
{
  public:
    Quadrotor(double mass, Eigen::Matrix3d inertia, double length,
      ...
      , _Kf_matrix((Eigen::Matrix<double, 3, 4>() << 0, 0, 0, 0, 0, 0, 0, 0,
                    _Kf, _Kf, _Kf, _Kf)
                     .finished())
      , _lKf(_l * _Kf)
    ...
    void update(Eigen::Vector4d &controls, double dt)
    {
        ...
        _last_ddq.segment(0, 3) =
          thrust / _m - _angular_velocity.cross(_velocity);
        _last_ddq.segment(3, 3) =
          _I_inv * (torque - _angular_velocity.cross(_I * _angular_velocity));
        ...
        Eigen::Vector4d quat_d     = 0.5 * get_quat_L(_orientation) *
                                 quadrotor::Value::Constant::H_mat *
                                 _angular_velocity;
    ...
    Eigen::Vector<double, 13> get_state()
    {
        ...
        state << _position, _orientation.w(), _orientation.vec(), _velocity,
          _angular_velocity;
        return state;
    }
  ...
  protected:
    ...
    const Eigen::Vector3d _gravity =
      Eigen::Vector3d(0, 0, -quadrotor::Value::Constant::g);
